...
var label_str binding.String

func main() {
	a := app.New()
	w := a.NewWindow("kkdyBenchmark")
	...
	label_str = binding.NewString()
	label_str.Set("")

	label := widget.NewLabelWithData(label_str)
	...
	name := widget.NewLabel("made by suulnnka")

	vbox := container.NewVBox(label,button,name)
...
func show_process() {
	atomic.AddInt32(&blockNum, 1)
	coreNumber := runtime.NumCPU()
	coreStr := "running on " + strconv.Itoa(coreNumber) + " core, "
	label_str.Set(coreStr + strconv.Itoa(int(blockNum)) + "/2148")
}
